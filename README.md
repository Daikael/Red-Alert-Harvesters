...
5. Optional: Mods → Startup → **Automatic harvester testing** (restart). That turns on the slow chunk index **and** physical auto-drive (pathfinder + car physics, no teleport). Open a truck’s inventory: **Automatic operation** (default on) and **Pause when somebody jumps in** (default off) sit on the left. Closing the inventory must **not** turn auto off. Empty truck + auto on should roll to an indexed patch. Remotes:
