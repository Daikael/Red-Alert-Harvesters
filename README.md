...
- **2.1 experimental:** this branch / pack **`Red-Alert-Harvester_2.1.17`**.

## Development branch names

Tester-facing development branches are **just the three-part version**: `2.1.17`, then `2.1.18`, … Do not use `cursor/…` names. The Factorio mod `info.json` **name** is singular **`Red-Alert-Harvester`** (portal slug). The pack folder is **`Red-Alert-Harvester_<version>`**.
...
GitHub **Code → Download ZIP** produces a folder like **`Red-Alert-Harvesters-2.1.17`** (repo-prefixed, hyphen before the version). That is **not** a Factorio install folder. Use the GitHub **Release** zip **`Red-Alert-Harvester_2.1.17.zip`**.
...
| Folder in `mods/` | `Red-Alert-Harvester_2.1.17` |
| Zip in `mods/` | `Red-Alert-Harvester_2.1.17.zip` |
| GitHub archive folder | `Red-Alert-Harvesters-2.1.17` (repo-prefixed — **not** an install path) |

The rule is always `{info.json name}_{info.json version}` — here that is **`Red-Alert-Harvester_2.1.17`**.

`info.json` must sit at the **zip root** or **one folder deep** with that same folder name:

```text
Red-Alert-Harvester_2.1.17.zip
  Red-Alert-Harvester_2.1.17/
    info.json
    control.lua
    ...
```

or the same files at the zip root. A zip named `Red-Alert-Harvesters.zip`, `Red-Alert-Harvesters_2.1.17.zip`, or `Red-Alert-Harvesters-2.1.17.zip` will fail to load.
...
rm -rf /tmp/Red-Alert-Harvester_2.1.17 /tmp/Red-Alert-Harvester_2.1.17.zip
mkdir /tmp/Red-Alert-Harvester_2.1.17
rsync -a --exclude .git --exclude .vscode ./ /tmp/Red-Alert-Harvester_2.1.17/
cd /tmp && zip -r Red-Alert-Harvester_2.1.17.zip Red-Alert-Harvester_2.1.17
# copy Red-Alert-Harvester_2.1.17.zip into your Factorio mods folder
```

On Windows: copy the repo into a folder literally named `Red-Alert-Harvester_2.1.17`, then zip that folder.

## Smoke test (2.1 experimental)

1. Confirm the Mods list shows **C&C Harvesters** / `Red-Alert-Harvester` **2.1.17** with no load error **without** Factorio-Tiberium installed (2.1.12 failed assignID on `basic-solid-tiberium`; 2.1.15 failed AtlasBuilder on `transparent.png` at left_top=256x0). Use a **2.1 experimental** client, not 2.0 stable. `factorio_version` must be **2.1**.
