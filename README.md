...
This branch is the **Factorio 2.1 experimental/beta** line (pack `2.1.2`). It is **not** the live 2.0 stable pack on `master`.

- **2.0 stable players:** use `master` / `Red-Alert-Harvester_2.0.0`.
- **2.1 experimental:** this branch / `Red-Alert-Harvester_2.1.2`.
...
doesn't match the expected Red-Alert-Harvester_2.1.2.zip (case sensitive!)
```
...
| `info.json` `version` | `2.1.2` |
| Folder in `mods/` | `Red-Alert-Harvester_2.1.2` |
| Zip in `mods/` | `Red-Alert-Harvester_2.1.2.zip` |

The rule is always `{info.json name}_{info.json version}` — here that is **`Red-Alert-Harvester_2.1.2`**.

`info.json` must sit at the **zip root** or **one folder deep** with that same folder name:

```text
Red-Alert-Harvester_2.1.2.zip
  Red-Alert-Harvester_2.1.2/
    info.json
    control.lua
    ...
```

or the same files at the zip root. A zip named `Red-Alert-Harvesters.zip`, `Red-Alert-Harvester.zip`, or `Red-Alert-Harvesters_2.1.2.zip` will fail to load.
...
rm -rf /tmp/Red-Alert-Harvester_2.1.2 /tmp/Red-Alert-Harvester_2.1.2.zip
mkdir /tmp/Red-Alert-Harvester_2.1.2
rsync -a --exclude .git --exclude .vscode ./ /tmp/Red-Alert-Harvester_2.1.2/
cd /tmp && zip -r Red-Alert-Harvester_2.1.2.zip Red-Alert-Harvester_2.1.2
# copy Red-Alert-Harvester_2.1.2.zip into your Factorio mods folder
```

On Windows: copy the repo into a folder literally named `Red-Alert-Harvester_2.1.2`, then zip that folder (not the GitHub repo folder name).

## Smoke test (2.1 experimental)

1. Confirm the Mods list shows **C&C Harvesters** / `Red-Alert-Harvester` **2.1.2** with no load error. Use a **2.1 experimental** client, not 2.0 stable.
...
See `FACTORIO_2.1.md`: companion module bay, quality/drain scooping, Hybrid-drive electric→burner converter.
