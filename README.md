...
5. Optional: Mods → Startup → **Automatic harvester testing** (restart). That turns on the slow chunk index **and** physical auto-drive (pathfinder + car physics, no teleport). Open a truck’s inventory: **Automatic operation** (default on) and **Pause when somebody jumps in** (default off) sit on the left. Auto on + pause off: you ride as **passenger**, AI keeps rolling, WASD does nothing. Auto on + pause on: take the driver seat, AI yields, exit resumes. Uncheck Automatic operation (real click, inventory open) to drive. Enter must **not** dump you on the ground or turn auto off. Low-fuel auto trucks drive to a **fueled refinery** and pull burnables from the chest into the **fuel tank** (Ore Truck 2 slots, Tiberium 3; not the cargo trunk). Wire the refinery with red/green to **read chest contents** (enable inserters when iron-ore < N). A full truck **must still path home** even if another miner sits somewhere along the route (passing near a sibling is not a path fail). Several full trucks at one refinery dump **in turn** (no `no-empty-refinery` storm while the chest is free). Waiters stop clear of the dock; they must not ram the dumper. After loading a pad-storm save, trucks should leave within a few seconds (leftover path-blocker dummies are swept 8/tick near trucks/pads; they must not CTD load or permanently idle). Cluster three at one patch: they should leave / mine / go home on open ground without a mutual stuck storm. Lowest `unit_number` peels if they get close; the pad holder peels first at the dock. Nose-to-nose: the leader **reverses if the rear is clear, then soft-repaths** the same dest. From a stop, trucks **rotate in place** to face the next waypoint, then accelerate. Assignment is the **nearest indexed** patch (not capped at 8 chunks). Trees that sit in the way are mined or destroyed. Dock from the **north dump face**; ~7 tiles out still dumps/refuels. Remotes:
