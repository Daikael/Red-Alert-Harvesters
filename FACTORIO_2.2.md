...
**Stuck-state reboot:** `aiwatch.lua` is an ongoing heartbeat (every 30 ticks, staggered by `unit_number`) for auto-on trucks. Meaningful progress is a state change, ≥0.5-tile **accumulated** move from the last-progress snap (not per 30-tick sample), trunk/tank count delta, path id start/finish, waypoint index, pad claim, or scoop count. If a truck sits in an expecting-progress state with no progress for ~30–90 s (`FindingOre` / `MovingToLocation` 90 s; `FindingRefinery` / `FindingRefuel*` 60 s; mine / dump / refuel / approach 30–45 s), `RebootAI` clears pathfinder leftovers (path ids, this truck’s path-blockers, peer-avoid, forever `busy_until`) and invalid pad reservations, rebinds the burner inventory, and re-picks the next goal: full trunk / `filled` → dump; empty tank → refuel; else find ore. It does **not** hardcode go-home. `AfterLoad` runs the same goal pick so a bricked save does not wait the full timeout; it still must not `StartDrive` / `KickAuto` on the load tick. Skip reboot (same class as pathfinder-busy / `queued_for_pad`) while auto is off, pause-on-enter has the driver, `busy_until` / `try_again_later` / load grace is active (unless `busy_until` is a leftover forever lock), **`queued_for_pad` / pad-recheck wait**, **in-flight `path_id`**, **active `path`**, **peer reverse peel**, **animation `wait_ticks`**, **idle `FindingOre` (index)** / **idle `FindingRefuelRefinery`**, or a 60 s post-reboot cooldown is running. When a reboot *does* fire, `log` + a rate-limited Deck `game.print` include state, goal, last-progress/age, `queued_for_pad`, `busy_until`, and path-busy. True reboot streaks: attempt 1 is clear-only; attempt 2+ may `find_non_colliding_position` then `vehicle.teleport` (range doubles from 8, cap 64, hop cooldown 60 s, plain `reboot_streak` / `reboot_hop_until` ints on the `unit_number` row). Never hop while queued, while `get_driver()` is a player, or during pause-on-enter yield. Physical `request_path` stays the default path. Optional rate-limited `cncharvester.ai-rebooted` print.
