...
**Dump queue:** `FindingRefinery` **reserves before** `StartDrive`. Waiters stop and recheck every `PAD_RECHECK_TICKS = 30`; they toast `no-empty-refinery` only when every pad is **full** or missing — not when the pad is merely reserved. `StartDrive` no-ops when the same dest already has a `path_id`/`path` (no per-tick `request_path` / blocker spawn). Pad lookup walks `storage.refineries`, not `find_entities_filtered`. `IsOccupied` reclaims a leaked lock. `holds_pad` is only `reserved_by == unit_number` (a saved `targetRefinery` is not a claim). Non-holders abort if they wander onto an occupied pad. The pad holder has peel priority so queued trucks do not ram the dumper. Dump inserts if the chest is not full (no 21-empty-stack gate). UnReserve on dump-done / path-give-up / leave.
