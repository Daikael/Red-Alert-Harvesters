...
**2.0.77 compat:** `LuaEntity.commandable` is nil on `type=car`. Autopilot fields are spidertron-only. Implementation is `surface.request_path` (the unit pathfinder, used with the **car** `collision_box` and `path_resolution_modifier = 0`) then `vehicle.riding_state`. A biter-sized box + coarse grid was tried and **reverted**: those waypoints do not fit a 2.8-wide car and exhausted `OnPathFail` on 2.0.77. `max_gap_size` stays 0. Prototype `collision_box` is centered at 0,0; car `collision_mask` **union** `cncharvester-peer`; `entity_to_ignore` = **self only**; `can_open_gates`; `allow_destroy_friendly_entities=false`. The unit pathfinder does **not** reliably treat other cars as obstacles, so each request may place hidden `cncharvester-path-blocker` simple-entities (4×4 tile box, **not** `type=car`, **not** a character) on **local** sibling trucks (within 32 tiles of start or goal; never the whole surface; **cap 16**; **reuse/teleport** the truck’s existing dummy list instead of destroy+create) until the async path returns, then **destroy** them. Hitch helper `teleport` in `modulebay.lua` is **not** autonomy — it keeps the slave drill on the truck (`raise_built = false` on create).
...
**Dump queue:** `FindingRefinery` **reserves before** `StartDrive`. Waiters stop and set `queued_for_pad` (refinery `unit_number`) plus `busy_until` (tick) on the harvester record in `storage` — **no callbacks**, **no** `StartDrive`. Pads are indexed in `storage.rah_pads` by `unit_number` only (`waiters` / `reserved_by` / `free_tick` — never raw entity refs). Recheck every `PAD_RECHECK_TICKS = 30`, or earlier for **only the next claimant** when the holder UnReserves (`free_tick` + lowest waiter id). Several full trucks on one reserved facility is **intentional**: one holder dumps, the others wait silently until the pad frees, then **one** claims and `StartDrive`s. Toast `no-empty-refinery` only when every pad is **full** or missing — not when the pad is merely reserved. `StartDrive` / `KickAuto` no-op for a waiter (`queued_for_pad` and not `holds_pad`). Pad lookup walks `storage.refineries`, not `find_entities_filtered`. `IsOccupied` reclaims a leaked lock. `holds_pad` is only `reserved_by == unit_number`. Non-holders abort onto an occupied pad into the same quiet queue. The pad holder has peel priority so queued trucks do not ram the dumper. Dump inserts if the chest is not full (no 21-empty-stack gate). UnReserve on dump-done / path-give-up / leave. Home `OnPathFail` burns the repath budget and `raise_stuck_alert` only while **holding** a reserved pad (or truly no pads / all-full). Quiet queue ≠ yellow/red stuck.

**Fuel-low:** `HybridDrive.potential_joules` below **8 MJ** (10% of the 80 MJ pool) → drive to a fueled refinery (`FindingRefuelRefinery` → `ApproachedForRefuel` → `Refueling`). Tick calls `MaybeReturnHome` even when `CheckFuel` is false so an empty Ore Truck still starts that trip. On arrival the refinery chest transfers **convertible burnables** (coal/wood/chemical; not nuclear / hybrid-charge) into `vehicle.burner.inventory` (Ore Truck **2** slots, type-2 **3**) until the tank cannot take more or the chest has no convertible left. `potential >= 8 MJ` with **zero items moved** is not success (solar/grid can sit above the trip threshold). An empty tank is never treated as full. Transfer tries slot walk, `get_contents`, `get_item_count`, then `entity.remove_item`. Then `convert_inventory_fuels` fills the hybrid pool to the 4 MJ working floor (leftover solids stay in the tank). Do not use `defines.inventory.fuel` as the tank — on cars that index can be cargo. Belt drop still withholds one fuel stack so inserters cannot empty the chest of truck fuel. No convertible fuel → toast `no-fuel-refinery` and try another refinery. Cargo full → drive to an unoccupied refinery. **Impact** damage is ignored (rocks). Other damage → drive home.

**Old-save recovery:** `storage.rah_migration.rev` (see `migrate.lua`) is a one-shot on `on_configuration_changed` and the first tick after `on_load`. Same-version zip overwrites often skip Factorio’s version-changed event, so the first-tick path is required. It clears ghost pad/refuel/path state and sends auto-on trucks with an empty tank back to the pad. It does not write `auto_enabled` / pause / cargo / positions. **Quit Factorio fully after overwriting the zip** — reload-save without restart can keep the previous Lua chunk. `on_load` still must not mutate `storage`.

**Stuck-state reboot:** `aiwatch.lua` is an ongoing heartbeat (every **90** ticks, staggered by `unit_number`, **max 3 full evals/tick**) for auto-on trucks. Cheap skip classes (`queued_for_pad`, in-flight `path_id` / active `path`, peer peel, idle `FindingOre`/`FindingRefuelRefinery`, `wait_ticks`, short `busy_until`) run **before** any inventory or `find_non_colliding_position`. Meaningful progress is a state change, ≥0.5-tile **accumulated** move from the last-progress snap (position-only first), path id start/finish, waypoint index, pad claim, or scoop count. Trunk/tank `get_item_count` runs **only at timeout** to confirm a sitting dump/refuel is not still progressing. If a truck sits in an expecting-progress state with no progress for ~30–90 s (`FindingOre` / `MovingToLocation` 90 s; `FindingRefinery` / `FindingRefuel*` 60 s; mine / dump / refuel / approach 30–45 s), `RebootAI` clears pathfinder leftovers (path ids, this truck’s path-blockers, peer-avoid, forever `busy_until`) and invalid pad reservations, rebinds the burner inventory, and re-picks the next goal: full trunk / `filled` → dump; empty tank → refuel; else find ore. It does **not** hardcode go-home. `AfterLoad` runs the same goal pick so a bricked save does not wait the full timeout; it still must not `StartDrive` / `KickAuto` on the load tick. Skip reboot (same class as pathfinder-busy / `queued_for_pad`) while auto is off, pause-on-enter has the driver, `busy_until` / `try_again_later` / load grace is active (unless `busy_until` is a leftover forever lock), **`queued_for_pad` / pad-recheck wait**, **in-flight `path_id`**, **active `path`**, **peer reverse peel**, **animation `wait_ticks`**, **idle `FindingOre` (index)** / **idle `FindingRefuelRefinery`**, or a 60 s post-reboot cooldown is running. When a reboot *does* fire, Deck gets a short rate-limited print; long reason fields + `log()` are `DEBUG_REASON` only. True reboot streaks: attempt 1 is clear-only; attempt 2+ may `find_non_colliding_position` then `vehicle.teleport` (range 8→**16 cap**, precision **1**, hop cooldown 60 s **even on a miss**, plain `reboot_streak` / `reboot_hop_until` ints on the `unit_number` row). Never hop while queued, while `get_driver()` is a player, or during pause-on-enter yield. Physical `request_path` stays the default path. Optional rate-limited `cncharvester.ai-rebooted` print.
...
- **Auto on + pause off:** demote the driver to passenger on enter / every tick (`set_passenger` of the **same character** first, then clear driver if still occupied). Do **not** `player.driving = false`. Do **not** `create_entity{name="character"}`. `riding_state` runs with an empty driver seat. A passenger must not cancel `StartDrive` / Tick pathing. If the prototype has no passenger seat, still clear driver so AI keeps the wheel (brief ground exit) and print that pause-on-enter or auto-off is required. Player input must **not** write `auto_enabled=false`. Unconnected `type=character` leftovers (no `ent.player`) are swept near the truck; a player-owned character is never destroyed.
...
- **Do not write these fields in `on_load`.** Factorio CRC-checks `storage` and will refuse the save (`Detected modifications to the 'storage' table`). Missing keys mean auto ON / pause OFF until a later mutable event (tick / GUI) writes them. Empty + auto ON calls `KickAuto` from occupancy/toggle (repath or `FindingOre` → `StartDrive`), never from `AfterLoad`. Load forgets blocker Lua refs, drops stale pad claims, and idles `LOAD_GRACE_TICKS` (staggered) before any `request_path`. It must not mass-destroy path-blockers (hard CTD on 2.0.77). After grace, `tick_purge_blockers` destroys `PURGE_PER_TICK = 8` leftovers within `PURGE_RADIUS = 32` of one truck/pad per tick; `request_path` omits `cncharvester-peer` until that sweeper finishes so storm leftovers cannot wall every StartDrive.
...
| `harvester.lua` | State machine. `FindingOre` reads ChunkIndex (`PickIndexTarget`). `MovingToLocation` is physical AutoDrive. Per-truck `auto_enabled` / `pause_on_enter`. `RebootAI` / `WatchAI` for stuck self-heal. Teleport leftovers commented only. |
| `aiwatch.lua` | Throttled stuck heartbeat + `pick_goal` / `apply_reboot`. AfterLoad uses `prepare_after_load` (state only). |
