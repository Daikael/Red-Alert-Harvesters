...
**Dump queue:** `FindingRefinery` **reserves before** `StartDrive`. Waiters stop and set `queued_for_pad` (refinery `unit_number`) plus `busy_until` (tick) on the harvester record in `storage` — **no callbacks**, **no** `StartDrive`. Recheck every `PAD_RECHECK_TICKS = 30`. Several full trucks on one reserved facility is **intentional**: one holder dumps, the others wait silently until the pad frees, then one claims. Toast `no-empty-refinery` only when every pad is **full** or missing — not when the pad is merely reserved. `StartDrive` / `KickAuto` no-op for a waiter (`queued_for_pad` and not `holds_pad`). Pad lookup walks `storage.refineries`, not `find_entities_filtered`. `IsOccupied` reclaims a leaked lock. `holds_pad` is only `reserved_by == unit_number`. Non-holders abort onto an occupied pad into the same quiet queue. The pad holder has peel priority so queued trucks do not ram the dumper. Dump inserts if the chest is not full (no 21-empty-stack gate). UnReserve on dump-done / path-give-up / leave. Home `OnPathFail` burns the repath budget and `raise_stuck_alert` only while **holding** a reserved pad (or truly no pads / all-full). Quiet queue ≠ yellow/red stuck.
