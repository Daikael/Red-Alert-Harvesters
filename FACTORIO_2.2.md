...
**Peer exclusion:** `AutoDrive.PEER_EXCLUDE_TILES = 16` (half a chunk). `PickIndexTarget` skips a chunk if another tracked harvester is already assigned to it (and not going home) **or** is sitting within 16 tiles of the chunk center. Soft backup so two autos do not park on the same patch. Runtime `PEER_CLEARANCE_TILES = 4`: if a sibling is ahead inside that bubble, brake — never keep accelerating `riding_state` into a pinned truck. Paths that pass within 4 tiles of a peer or set `needs_destroy_to_reach` are rejected and use the existing stuck escalation (repath → other patch → home).
